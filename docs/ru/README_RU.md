...
15. [Обучение GPT-подобной LLM (опционально, с warm-start от энкодера)](#обучение-gpt-подобной-llm-опционально-с-warm-start-от-энкодера)
16. [Трекинг экспериментов в MLflow](#трекинг-экспериментов-в-mlflow)
17. [Тестирование](#тестирование)
18. [Docker и docker-compose](#docker-и-docker-compose)
19. [Адаптация под другой язык](#адаптация-под-другой-язык)
20. [Справочник по Makefile](#справочник-по-makefile)
21. [Решение проблем](#решение-проблем)
...
Отдельно для тестовых наборов, размеченных SVD: поскольку пары являются равномерно случайными, а не
подобранными истинно положительными парами, `recall@k` и `mrr@k` в получившемся
`metrics/eval_<lang>.json` находятся на уровне случайного угадывания по построению — единственный
содержательный сигнал качества для этого режима — корреляция Спирмена.
...
## Обучение GPT-подобной LLM (опционально, с warm-start от энкодера)

Флаг `--train-llm` (включается явно, по умолчанию выключен) добавляет ещё один этап после
получения финальных эмбеддингов: обучение небольшого каузального GPT-2-подобного декодера
(предсказание следующего токена) на том же корпусе и токенизаторе, что уже построены на более
ранних этапах пайплайна.

**Это не то же самое, что `src/langembed/llm_embed/`** (Ветка C в архитектурном сравнении этого
проекта, LLM как эмбеддер — предобученный декодер + LoRA-адаптеры, усреднённые в эмбеддинги
предложений). Новый этап работает в обратном направлении: он *производит* небольшую генеративную
модель, а не потребляет готовую.

**Warm-start касается только эмбеддингов.** Двунаправленный энкодер (`artifacts/encoder_<lang>`)
и новый каузальный декодер несовместимы по весам для своих блоков внимания, поэтому копируется
только таблица эмбеддингов токенов — всё остальное в GPT-модели обучается с нуля. Это требует,
чтобы `hidden_size` GPT-модели точно совпадал с `hidden_size` энкодера; `run_pipeline.py`
удерживает оба значения равными `256` именно по этой причине.

```bash
python scripts/run_pipeline.py --lang gu --raw-input data/raw/gu_nllb.txt --train-llm

# с явным ограничением по времени обучения (по умолчанию: 25 минут, как у --pretrain-minutes)
python scripts/run_pipeline.py --lang gu --raw-input data/raw/gu_nllb.txt \
    --train-llm --llm-minutes 15
```

Обученная модель сохраняется в `artifacts/gpt_<lang>/` и может быть переобучена отдельно, как
только появится `configs/<lang>/llm.yaml` (создаётся пайплайном при первом запуске):

```bash
python -m langembed.llm.train_gpt --config configs/gu/llm.yaml
```

### Проверочный запуск (реальная GPU-инфраструктура, `ur`)

Функциональность была проверена целиком на реальном хосте развёртывания — Docker + GPU, а не
только на CPU-фикстуре e2e-теста — с использованием уже готового энкодера и корпуса для `ur`
(построение корпуса и токенизатора не потребовалось — оба артефакта уже существовали после
предыдущего полного запуска пайплайна для `ur`):

- **Источник warm-start:** `artifacts/encoder_ur` — `hidden_size=256`, `vocab_size=16000`, точно
  совпадает с конфигурацией GPT-модели, поэтому копирование таблицы эмбеддингов прошло без
  несовпадения размерностей.
- **Корпус:** `data/corpus_ur.txt`, 398 955 предложений (тот же корпус, на котором обучались
  энкодер и SimCSE-модель `ur`).
- **Форма запуска:** сначала smoke-запуск (50 шагов) для проверки работоспособности кода, затем
  один ограниченный реальный запуск — `max_steps=400`, `per_device_train_batch_size=16`,
  `learning_rate=0.0005`, `warmup_steps=50` — намеренно небольшой, чтобы тест не конкурировал за
  GPU/память с намного более крупным запуском пайплайна (`gu`, ~13,8 млн предложений), который
  на том же хосте в это время завершался.
- **Результат:** ошибка обучения снизилась с `6.746` до `5.769` за 400 шагов
  (`train_samples_per_second=1386`, `train_steps_per_second=86.61`, ~4,6 секунды вычислений
  обучения) — реальный, монотонный сигнал обучения, а не плоский/сломанный прогон.
  `artifacts/gpt_ur/` получился как полноценный, загружаемый чекпоинт (`config.json`,
  `model.safetensors`, `tokenizer.json`, `tokenizer_config.json`, `generation_config.json`),
  проверено — загружается через `GPT2LMHeadModel.from_pretrained`.
- **Что это подтверждает:** копирование весов при warm-start, цикл каузального обучения LM,
  сохранение/загрузку чекпоинта и весь путь вызова CLI/подпроцесса — всё это корректно работает
  на реальных артефактах на реальной инфраструктуре — то же самое, что уже проверял CPU e2e-тест
  (крошечная фикстура, `hidden_size: 128`), теперь подтверждено и на реальном GPU-пути развёртывания.
- **Чего это НЕ подтверждает:** качество генерации. 400 шагов — это проверка механики, а не тест
  качества — получившийся чекпоинт `artifacts/gpt_ur/` совершенно недообучен для связной генерации
  текста. Для реального запуска нужен реальный бюджет `--llm-minutes` (тот же калиброванный,
  ограниченный по времени подход, что уже используется на этапе pretrain энкодера), который этот
  проверочный запуск намеренно пропустил, чтобы минимизировать нагрузку на ресурсы.
